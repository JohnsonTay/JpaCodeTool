...
public   class CodeGenerator {
	...
	private String pkgPrefix = "com.cly";
	...
	public void initCodeTool(List<Column> lsColumns) throws Exception {
		classVar = StringUtil.toBeanPatternStr(tableName.substring(tablePrefix.length()-1));
		...
		jspDir = (StringUtil.isEmpty(module) ? "" : ("/" + module))
				+ (StringUtil.isEmpty(subModule) ? "" : ("/" + subModule));
		tool.put("tableName", tableName.toLowerCase());
		...
		if (pkCnt > 1 || pkCnt ==0) {
			...
			tool.put("keyType", keyType);
			tool.put("keyVar", keyVar);
			tool.put("comments", column.getComments());
		} else {
...
//		tool.put("stringUtil",stringUtil );
	 ...
	 */
	@Test
	public void createRepositoryFile() throws Exception {
		String classNameWithPkg = pkgPrefix + ".repository." + className + "Repository";
		String javaFile = javaSource + "/"
				+ FileUtil.getJavaFileName(classNameWithPkg);
		tool.setTmplFile("RepositoryInterface.tmpl");
		tool.setAbsolutePath(javaFile);
		tool.createFileByTmpl();

		classNameWithPkg = pkgPrefix + ".repository.hibernate." + className
				+ "RepositoryHibernate";
		javaFile = javaSource + "/" + FileUtil.getJavaFileName(classNameWithPkg);
		tool.setTmplFile("RepositoryImpl.tmpl");
		tool.setAbsolutePath(javaFile);
		tool.createFileByTmpl();
	}
